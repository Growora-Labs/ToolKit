    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '64px 16px 0' }}>
      ...
      <section style={sectionStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--r-l)', overflow: 'hidden' }}>
          {[
            { q: 'What is the difference between random text and Lorem Ipsum?', a: 'Lorem Ipsum is a specific Latin text derived from Cicero\'s de Finibus Bonorum et Malorum, scrambled to be unreadable. It has been the standard design placeholder since the 1500s and is immediately recognisable as placeholder content to every designer and developer. Random English text, by contrast, uses real English words assembled in random combinations. The key difference in practice: Lorem Ipsum signals "this is definitely placeholder, ignore the content"; random English text looks like real content at a glance, making it better for testing how designs respond to real-world text and for seeding databases with data that passes basic validation.' },
            { q: 'Can I use random text for testing inputs in forms?', a: 'Yes — form field testing is one of the best use cases for a random text generator. Paste words of varying lengths into name fields, email subject lines, comment boxes, and search inputs to verify that your UI handles short, medium, and long values correctly. For systematic boundary testing, generate strings of exactly the minimum and maximum allowed lengths. Note that random text will not test special character handling (XSS, SQL injection, emoji) — for security testing you need purpose-crafted adversarial inputs, not random words.' },
            { q: 'Is random text useful for SEO testing?', a: 'Random English text is useful for testing the infrastructure around your SEO implementation — checking that meta tags render correctly, that sitemaps generate properly, that structured data validates — but it is not useful for testing actual SEO performance. Search engines evaluate content quality, topical relevance, and semantic depth. Random text, even if it uses real English words, has no topical coherence and would rank for nothing and should never appear on a public-facing page that you want indexed.' },
            { q: 'How is the random text generated — is it a Markov chain or AI?', a: 'This tool uses a curated word list combined with simple grammatical rules to produce plausible-looking sentences without any language model or AI. The words are drawn from a pool of common English vocabulary, assembled into sentences with a subject-verb-object structure, and grouped into paragraphs. The result looks convincing at a glance but has no semantic coherence — adjacent sentences are not topically related. This is intentional: the goal is neutral placeholder content, not meaningful prose.' },
            { q: 'Can I generate random text in languages other than English?', a: 'This tool generates English text only. For placeholder text in other languages, use a Lorem Ipsum generator (Latin is language-neutral as a placeholder) or a language-specific placeholder generator. Many languages also have their own variants of Lorem Ipsum — for example, Cicero\'s text has been adapted into placeholder versions for Chinese (乱数假文), Japanese, Arabic, and other scripts. For database testing with international content, the most reliable approach is to use a small sample of real native-language text rather than generated placeholder content.' },
            { q: 'What is the maximum amount of text I can generate at once?', a: 'There is no hard limit enforced by this tool — generation runs entirely in your browser using JavaScript. In practice, you can generate thousands of words or hundreds of paragraphs without any noticeable delay. For very large volumes (millions of words), a command-line approach using a library like Faker.js (Node.js), Faker (Python), or a similar library in your language of choice will be more efficient, as it avoids the browser memory limitations and can pipe output directly to a file or database.' },
            { q: 'Is the generated text copyrightable?', a: 'No. Randomly assembled text that lacks authorial creative expression is not protectable by copyright. The text is generated by an algorithm combining words from a vocabulary list — there is no human creative act that would establish copyright. You can use generated text freely in any commercial or non-commercial context, including publishing it as placeholder content (though you should replace it with real content before going live), using it as test data in proprietary software, or including it in open-source projects.' },
            { q: 'How do I generate random text programmatically in my code?', a: 'Most languages have a Faker library that generates realistic-looking random data including text. JavaScript/TypeScript: @faker-js/faker — faker.lorem.paragraph(), faker.lorem.sentence(). Python: faker — from faker import Faker; fake = Faker(); fake.paragraph(). PHP: fzaninotto/faker — $faker->paragraph(). Ruby: faker gem — Faker::Lorem.paragraph. These libraries also generate random names, addresses, emails, phone numbers, and dozens of other data types — far more useful for comprehensive database seeding than a simple text generator.' },
          ].map(({ q, a }, i, arr) => (
