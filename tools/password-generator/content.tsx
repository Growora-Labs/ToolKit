            ...
            <div>
                ...
                <section style={{ marginBottom: 48 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--r-l)', overflow: 'hidden' }}>
                        {[
                            { q: 'Is it safe to generate passwords in a browser?', a: 'This generator uses window.crypto.getRandomValues() — the Web Crypto API — which draws randomness directly from the operating system\'s CSPRNG (Cryptographically Secure Pseudo-Random Number Generator). It is the same source of randomness used for TLS key generation. The generation runs entirely client-side: no data is sent to any server, no passwords are logged, and the result is never stored anywhere outside your clipboard. The risk profile is equivalent to running a local password generator on your own machine.' },
                            { q: 'How long should my password actually be?', a: 'The NIST Digital Identity Guidelines (SP 800-63B, 2017) recommend a minimum of 8 characters for passwords chosen by users and at least 15 characters when system-generated. In practice, use at least 16 characters for regular accounts and 24+ for critical accounts like email, banking, and your password manager master password. Longer is always better — a 24-character all-lowercase password is exponentially harder to brute-force than an 8-character "complex" one with symbols.' },
                            { q: 'Should I use symbols in my password?', a: 'Symbols increase the character set size and therefore the entropy per character. Adding symbols to a 16-character password is beneficial. However, some systems have restrictions on which symbols they accept — particularly older banking and enterprise systems that reject characters like < > " \' or ;. If you generate a password and a site rejects it, try generating another without symbols. Never reduce length to work around symbol restrictions; a long all-alphanumeric password is still stronger than a short "complex" one.' },
                            { q: 'What is entropy and how much do I need?', a: 'Entropy is the measure of randomness in bits — higher entropy means a harder-to-guess password. Entropy is calculated as log₂(character_set_size ^ length). A 16-character password using all character types (uppercase + lowercase + digits + symbols = ~94 characters) has about 105 bits of entropy. 80 bits is the generally accepted minimum for strong passwords against offline attacks. 128 bits (achievable at about 20 characters with full character sets) is considered very strong and future-proof against near-term computing advances.' },
                            { q: 'How often should I change my passwords?', a: 'Modern security guidance (NIST 2017 onwards) no longer recommends periodic mandatory password changes. Forced rotations encourage users to make predictable changes (Password1 → Password2) and create cognitive load that leads to weaker choices. Instead, change passwords when: (1) there is evidence of a breach — check haveibeenpwned.com, (2) you suspect compromise, (3) you shared a password and no longer trust the recipient. Using unique generated passwords for every account eliminates most of the risk that periodic rotation was designed to address.' },
                            { q: 'Can two-factor authentication replace a strong password?', a: '2FA significantly reduces account takeover risk, but it does not replace the need for a strong unique password. Most 2FA implementations can be bypassed if an attacker has your password and can perform real-time phishing (a "man-in-the-middle" attack that relays your 2FA code). A strong password prevents the attacker from getting to the 2FA prompt in the first place. The ideal defence is layered: unique strong password + 2FA + a phishing-resistant 2FA method (hardware security key or passkey) for the most critical accounts.' },
                            { q: 'What is the difference between a password manager and browser-saved passwords?', a: 'Browser-saved passwords (Chrome, Firefox, Safari) are convenient but have important limitations: they are accessible to anyone with physical access to your unlocked device, they sync across devices without strong zero-knowledge encryption in some browsers, they can be extracted by malware with browser access, and they offer no breach monitoring. A dedicated password manager (1Password, Bitwarden, Dashlane) encrypts your vault with your master password, uses a zero-knowledge architecture (the provider cannot see your passwords), offers cross-browser support, and includes breach monitoring and password health reports.' },
                            { q: 'What should I do if I think my password was compromised in a data breach?', a: 'Immediately change the password for the affected account and for any other account where you reused the same password. Check haveibeenpwned.com to see which breaches contain your email address. Enable two-factor authentication on the affected account if you have not already. If the breached service was your email provider, act immediately — email account access enables password reset on most other services, making it the most sensitive account to secure. Generate new unique passwords with this tool and store them in a password manager.' },
                        ].map(({ q, a }, i, arr) => (
