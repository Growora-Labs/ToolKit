  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '64px 16px 0' }}>
      <div>
        ...
        {/* ── FAQ ─────────────────────────────────────── */}
        <section style={{ marginBottom: 48 }}>
          <h2 style={{ ...h2, marginBottom: 24 }}>Frequently asked questions</h2>
          <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--r-l)', overflow: 'hidden' }}>
            {[
              { q: 'What algorithm does this tool use for diffing?', a: 'This tool uses the LCS (Longest Common Subsequence) algorithm, which is the same approach used by the Unix diff command, Git\'s diff engine, and GitHub\'s code review interface. LCS finds the minimal set of insertions and deletions needed to transform one text into another. The output is a "minimal diff" — no line is marked as changed if it could be considered equal. This produces the most informative, least noisy output for human review.' },
              { q: 'What is the difference between character-level and line-level diff?', a: 'A line-level diff (like this tool) compares texts line by line — a whole line is either added, removed, or unchanged. This is the standard for code review and is most readable when changes are confined to specific lines. A character-level diff (also called a word diff or inline diff) highlights the exact characters that changed within a modified line. For prose editing, character-level diff is more informative; for code, line-level is usually clearer. Some tools offer both: git diff --word-diff applies character-level highlighting within the context of a line-level diff.' },
              { q: 'Can I use this tool to diff code files?', a: 'Yes — this tool works with any text content including source code, configuration files, JSON, YAML, SQL, HTML, and Markdown. For the best experience diffing code: (1) paste the raw source without any syntax highlighting markup, (2) enable "Ignore whitespace" if the only changes are indentation or formatting, and (3) use the unified view for small changes and the split view for large block-level rewrites. For very large files (thousands of lines), a local diff tool (VS Code diff editor, git diff) will perform better than a browser-based tool.' },
              { q: 'Why does the diff show many changes when I only changed a few words?', a: 'The most common cause is whitespace differences. If one version was pasted from a text editor that uses 4-space indentation and the other uses 2-space indentation, every indented line will appear as changed. Enable "Ignore whitespace" to suppress these formatting-only differences. A second cause is line ending differences — Windows uses CRLF (\\r\\n) and Unix uses LF (\\n). If the texts were created on different systems, every line may appear changed due to the invisible \\r character. This tool normalises line endings before diffing to prevent false positives.' },
              { q: 'How do I diff two JSON files properly?', a: 'Paste both JSON texts through the JSON Formatter first (prettify with consistent indentation and the same number of spaces), then paste the formatted versions into the diff tool. This eliminates false positives from different indentation styles. If key order differs between the two JSON objects, you may still see many apparent changes even if the data is equivalent — JSON does not guarantee key order, and a diff tool has no way to recognise that {"a":1,"b":2} and {"b":2,"a":1} are semantically identical.' },
              { q: 'Can I copy the diff output and share it?', a: 'The "Copy diff" button copies the diff in standard unified diff format — the same format produced by Unix diff -u and git diff. This format is universally understood by developers and can be pasted into GitHub comments (it will be rendered with syntax highlighting), Jira or Linear tickets, Slack messages, and email. The format includes − and + line prefixes and context lines that make the change immediately readable without needing the original and modified files.' },
              { q: 'Does this tool work for comparing non-code text like articles or documents?', a: 'Yes — text diff is equally useful for prose writing, legal documents, academic papers, and any other text where you need to track revisions. Paste the original draft and the revised draft, and the diff highlights exactly which sentences were added, removed, or (at the line level) rewritten. For document editing contexts, "Ignore case" can be useful if one version changed capitalisation for stylistic reasons but the content is otherwise identical.' },
              { q: 'Is there a size limit on the texts I can compare?', a: 'There is no enforced limit — the diff runs entirely in JavaScript in your browser. For small-to-medium texts (up to a few hundred kilobytes), diffing is instantaneous. For very large inputs (megabytes of text or files with tens of thousands of lines), the LCS algorithm\'s time complexity is O(n×m) where n and m are the line counts — this can become slow. For large code files, use a local tool: VS Code\'s built-in diff editor (File → Compare Active File with...) or git diff are more efficient for very large inputs.' },
            ].map(({ q, a }, i, arr) => (
