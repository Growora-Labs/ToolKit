        ...
        <div>
          ...
          <section style={{ marginBottom: 48 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--r-l)', overflow: 'hidden' }}>
              {[
                { q: 'How is "word" defined in the word counter?', a: 'This tool counts words using whitespace tokenisation — any sequence of non-whitespace characters separated by a space, tab, or newline counts as one word. This matches the behaviour of most word processors and online word counters. Implications: hyphenated words like "well-known" count as one word. A URL like "https://example.com/very/long/path" counts as one word. Numbers count as words. Punctuation attached to a word ("hello," or "(example)") is counted as part of that word token.' },
                { q: 'Does the character count include spaces?', a: 'This tool shows both: "Characters (with spaces)" counts every character including spaces, tabs, and newlines — this is the metric used by Twitter/X, Instagram, and most character-limited platforms. "Characters (without spaces)" counts only the non-whitespace characters — useful for typesetting estimates and some academic submission requirements that specify character limits without spaces. When a platform says "character limit", it almost always means characters with spaces.' },
                { q: 'How is a sentence counted?', a: 'Sentences are counted by splitting on sentence-ending punctuation: periods (.), exclamation marks (!), and question marks (?). Abbreviations (Dr., U.S.A., etc.) cause common false positives — this tool uses heuristics to reduce miscounting, but complex abbreviation-heavy text may show a slightly inflated sentence count. Dialogue with multiple question marks or exclamation points in sequence counts each as a sentence boundary. The sentence count is an approximation useful for readability estimates, not a precise grammatical parse.' },
                { q: 'What is keyword density and why does it matter for SEO?', a: 'Keyword density is the percentage of times a specific word or phrase appears relative to the total word count. For example, a 1,000-word article that uses "content marketing" 10 times has a keyword density of 1% for that phrase. SEO guidelines generally suggest targeting 1–2% density for your primary keyword — below that, your page may not rank for the term; above that (keyword stuffing), search engines may penalise the page. Use the keyword density panel in this tool to check your distribution and identify over-used or under-used terms.' },
                { q: 'Does the word counter work with HTML or Markdown?', a: 'This tool counts words in plain text — HTML tags and Markdown syntax are counted as text tokens, not stripped. If you paste raw HTML like <h1>Hello World</h1>, the counter will count "h1", "Hello", "World", and "/h1" as four words. For accurate word counts of HTML or Markdown documents, either paste the rendered plain text (copy from the browser or rendered preview) or strip the markup before counting. The reading time estimate will also be inflated if markup is included.' },
                { q: 'Why does my word count differ from Microsoft Word?', a: 'Different tools use different tokenisation rules. Microsoft Word treats hyphenated words as two words in some cases. It also counts footnotes, headers, and text boxes separately by default (File → Properties → Statistics shows total count including these). Most online word counters, including this one, count plain whitespace-delimited tokens and exclude formatting metadata. Differences of 1–3% between tools are normal for most documents. If precise academic submission counts are required, verify with the submission system\'s own word count tool.' },
                { q: 'What does the Flesch-Kincaid readability score mean?', a: 'The Flesch Reading Ease score rates text on a scale of 0–100. Higher scores indicate easier reading: 90–100 is very easy (5th grade level), 60–70 is standard (8th–9th grade, the target for most web content), 30–50 is difficult (college level), and below 30 is very difficult (professional or academic). The score is calculated from average sentence length and average syllable count per word. Long sentences and multi-syllable words lower the score. Most blog content targets 60–70; technical documentation typically scores 30–50.' },
                { q: 'Is there a maximum text size the counter can handle?', a: 'There is no enforced size limit — all processing runs locally in your browser with no server round-trips. In practice, the counter handles documents of hundreds of thousands of words without performance issues on modern devices. For very large documents (entire books or large codebases), performance depends on your device. The real practical limit is browser clipboard capacity when pasting — most operating systems limit clipboard content to a few hundred megabytes, which far exceeds any realistic document size.' },
              ].map(({ q, a }, i, arr) => (
