  ...
  "display": "standalone"
                ...
                <section style={sectionStyle}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--r-l)', overflow: 'hidden' }}>
                        {[
                            { q: 'Do I really need all those different sizes, or is one favicon.ico enough?', a: 'For a minimal setup, a single favicon.ico in the site root covers the basics — every browser will use it as the fallback. However, you will get a blurry or pixelated icon on Retina/HiDPI displays, a poor iOS home screen icon, and a failed PWA installability check. For any site targeting mobile users or presenting itself as a professional product, you need at minimum: favicon.ico (16×16 + 32×32 bundled), apple-touch-icon.png (180×180), and two manifest icons (192×192 and 512×512). The effort is one-time and the quality difference is immediately visible.' },
                            { q: 'What image should I use as the source for my favicon?', a: 'The ideal favicon source is your logo or brand mark in SVG format — a vector file that can be cleanly downscaled to any pixel size. If you only have a PNG logo, use a high-resolution version (at least 512×512 pixels) so that downscaling does not introduce blur. Avoid starting from a JPEG due to compression artifacts that look terrible at small sizes. The most common approach: export a simplified, high-contrast version of your logo at 512×512 pixels and use that as the source for all generated sizes.' },
                            { q: 'Why is my favicon not updating after I change it?', a: 'Browsers cache favicons aggressively — often for weeks — independently of the page\'s cache headers. To force an update: hard-reload the page (Ctrl+Shift+R / Cmd+Shift+R), clear your browser cache, or open DevTools → Network → disable cache, then reload. In production, cache-bust your favicon URLs by appending a version query parameter: <link rel="icon" href="/favicon.ico?v=2">. When you update your favicon, increment the version number to force all visitors to download the new version.' },
                            { q: 'Should I use an SVG favicon instead of PNG or ICO?', a: 'SVG favicons are supported in all modern browsers (Chrome 80+, Firefox 41+, Safari 14+) and offer significant advantages: infinite scalability, dark mode support via CSS media queries inside the SVG, and smaller file size for complex icons. The main limitation is that SVG favicons are not supported in older browsers and do not work as Apple touch icons. Best practice for 2024: declare an SVG favicon as the primary icon with a PNG or ICO fallback for older browsers: <link rel="icon" href="/favicon.svg" type="image/svg+xml"> followed by <link rel="icon" href="/favicon.ico" sizes="any">.' },
                            { q: 'What is the Web App Manifest and do I need it?', a: 'The Web App Manifest (manifest.json or site.webmanifest) is a JSON file that tells the browser how to display your website when it is installed as a Progressive Web App. It specifies the app name, icon set, theme colour, background colour, display mode, and start URL. You need it if you want your site to be installable on mobile home screens and pass the PWA installability criteria checked by Lighthouse. Without a manifest, Android Chrome will not offer the "Add to Home Screen" prompt, and your PWA will show a generic browser icon instead of your branded one.' },
                            { q: 'What is the apple-touch-icon and how is it different from a favicon?', a: 'The apple-touch-icon is the icon iOS Safari uses when a user saves your website to their iPhone or iPad home screen. It is a PNG image at 180×180 pixels, declared with <link rel="apple-touch-icon" sizes="180x180" href="/apple-touch-icon.png">. Unlike a favicon (which appears in browser tabs), the apple-touch-icon appears full-size on the home screen alongside app icons. Apple applies rounded corners and a gloss effect automatically on older iOS versions; modern iOS uses the image as-is with system-level rounded corners.' },
                            { q: 'Can I use an emoji as a favicon without any image files?', a: 'Yes, using an inline SVG data URI in the <link> tag: <link rel="icon" href="data:image/svg+xml,<svg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 100 100\'><text y=\'.9em\' font-size=\'90\'>🛠️</text></svg>">. This requires zero image files and works in Chrome 80+, Firefox 75+, and Safari 14+. The emoji is rendered by the OS emoji font and looks correct on all platforms. The limitation: it does not provide a home screen icon for iOS — you still need an apple-touch-icon.png for that use case.' },
                            { q: 'How do I set a different favicon for dark mode?', a: 'SVG favicons support CSS media queries including prefers-color-scheme. Inside your favicon SVG, you can define two versions of the icon — one for light mode and one for dark mode — and the browser will automatically switch between them based on the system setting. Example: <style>@media (prefers-color-scheme: dark) { .icon { fill: white; } } @media (prefers-color-scheme: light) { .icon { fill: black; } }</style> inside the SVG file. This technique works in Firefox 60+ and Chrome 100+, and is the recommended approach for icons that would disappear against the browser\'s dark theme chrome.' },
                        ].map(({ q, a }, i, arr) => (
