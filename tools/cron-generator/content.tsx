            ...
            <div>
                ...
                <section style={sectionStyle}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--r-l)', overflow: 'hidden' }}>
                        {[
                            { q: 'What does * * * * * mean in cron?', a: 'Five asterisks means "run every minute". Each asterisk matches any value in its respective field: any minute, any hour, any day of month, any month, any day of week. It is the most permissive expression — the job will fire once per minute, 60 times per hour, 1,440 times per day. It is useful for health-check pings and rapid polling during development, but should never be used for expensive operations like database queries or external API calls in production.' },
                            { q: 'How do I run a cron job every 5 minutes?', a: 'Use the step operator with the wildcard: */5 * * * *. The slash means "every N", so */5 in the minute field means "every 5 minutes starting from 0": at :00, :05, :10, :15, and so on. Similarly, */15 runs every 15 minutes, */30 runs every 30 minutes. You can also use a range with step: 0-30/5 * * * * runs every 5 minutes but only within the first half of each hour (:00, :05, :10, :15, :20, :25, :30).' },
                            { q: 'How do I schedule a job to run at a specific time on weekdays only?', a: 'Combine the time fields with a day-of-week constraint. To run at 9:30 AM every weekday: 30 9 * * 1-5. The day-of-week field uses 0–6 where 0 is Sunday (on most systems; some systems also accept 7 as Sunday). Monday through Friday is 1-5. To also exclude holidays, you need application-level logic — cron has no concept of public holidays or corporate calendars.' },
                            { q: 'What happens if a cron job takes longer than its scheduled interval?', a: 'By default, the cron daemon starts a new instance of the job at the scheduled time regardless of whether the previous instance is still running. This means two or more instances can overlap, which can cause data corruption, duplicate emails, or race conditions if the job modifies shared state. To prevent overlap: use a lock file or distributed mutex (Redis-based locks, PostgreSQL advisory locks) to ensure only one instance runs at a time, or use a cron implementation that supports the concurrencyPolicy: Forbid setting (available in Kubernetes CronJobs).' },
                            { q: 'How do I run a cron job only once on a specific date?', a: 'Combine specific values in all five fields. To run once at 14:00 on March 15: 0 14 15 3 *. This fires at 2:00 PM on the 15th of March every year — not just once. To truly run only once, you need to either delete the crontab entry after execution, use a one-shot scheduler service (AWS EventBridge one-time rules, systemd .timer units with OnCalendar), or implement a "has this run before" check inside your job script and exit early if it has.' },
                            { q: 'Does cron support seconds-level scheduling?', a: 'Standard Unix cron does not support sub-minute scheduling — the smallest unit is one minute. If you need jobs that run every few seconds, you need a different tool: a language-level scheduler (node-cron with seconds support, APScheduler in Python), a message queue with delayed delivery, or a dedicated job scheduler like BullMQ or Celery Beat. AWS EventBridge Scheduler supports rates as low as 1 minute and can be augmented with Step Functions for sub-minute workflows.' },
                            { q: 'How do I see which cron jobs are currently scheduled on a Linux server?', a: 'To see your own user\'s crontab: crontab -l. To see another user\'s crontab (as root): crontab -u username -l. System-wide cron jobs live in /etc/crontab, /etc/cron.d/, /etc/cron.daily/, /etc/cron.hourly/, /etc/cron.weekly/, and /etc/cron.monthly/. Use cat /etc/crontab and ls /etc/cron.d/ to inspect them. For systemd-based systems, systemctl list-timers shows all active systemd timer units, which are the modern replacement for cron on many Linux distributions.' },
                            { q: 'Why did my cron job fail silently without any output or logs?', a: 'Cron jobs fail silently for several common reasons: (1) Environment differences — cron runs with a minimal environment (no PATH, HOME, or shell aliases). Your script may depend on commands or paths that are available in your interactive shell but not in cron\'s environment. Fix by using absolute paths for all commands. (2) No logging configured — redirect output explicitly: * * * * * /path/to/script.sh >> /var/log/myscript.log 2>&1. (3) Email not configured — cron sends output to the system mail of the running user; if no mail agent is configured, output is silently discarded. (4) Permission issues — the script may not be executable; run chmod +x /path/to/script.sh.' },
                        ].map(({ q, a }, i, arr) => (
