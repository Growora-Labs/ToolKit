    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '64px 16px 0' }}>
      ...
      <section style={sectionStyle}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', borderRadius: 'var(--r-l)', overflow: 'hidden' }}>
          {[
            { q: 'How accurate is the reading time estimate?', a: 'The estimate is accurate as a population average for silent, comprehension-oriented reading of narrative prose at the 238 WPM baseline. For an individual reader, actual time may vary by ±50% depending on reading speed, familiarity with the subject matter, and purpose (skimming vs. careful reading). Technical content with domain-specific vocabulary, code snippets, and complex diagrams is typically read 20–40% slower than general prose. The estimate is best treated as an "order of magnitude" signal — a 2-minute vs. 10-minute read — rather than a precise prediction.' },
            { q: 'Why do Medium and my own calculator show different reading times for the same article?', a: 'Platform differences in assumed reading speed account for most of the discrepancy. Medium uses 265 WPM; Substack uses approximately 200 WPM; dev.to uses 250 WPM; this calculator uses 238 WPM (the Brysbaert 2019 meta-analysis baseline). A 1,500-word article produces reading time estimates of: 5.6 min at 265 WPM, 7.5 min at 200 WPM, 6 min at 250 WPM, and 6.3 min at 238 WPM. None is objectively correct — each reflects a different editorial assumption about the target audience.' },
            { q: 'Should I display reading time on my blog or website?', a: 'Yes, for most content types — it consistently improves reader engagement. Medium found that displaying reading time increased click-through rates for articles in the 7–10 minute range, where users previously assumed the article was too long to start. The effect is strongest for long-form content where the absence of a reading time estimate would make readers uncertain about their time commitment. For very short pieces (under 2 minutes), reading time adds less value — the article is short enough that the title and first paragraph are sufficient signals.' },
            { q: 'Does reading time matter for SEO?', a: 'Reading time is not a direct ranking signal. However, the content depth that produces a longer reading time — comprehensive coverage, multiple sections, supporting examples, tables, and code snippets — does correlate with higher rankings for informational queries. More practically, articles with accurate reading time labels in meta descriptions and social previews have higher click-through rates from search results and social shares, which indirectly affects engagement signals that search engines monitor.' },
            { q: 'How do I add reading time to my blog or CMS?', a: 'Most static site generators have plugins: Gatsby has gatsby-remark-reading-time, Hugo has a built-in .ReadingTime variable, Eleventy has eleventy-plugin-reading-time, and VitePress calculates it automatically. For custom implementations, the formula is: Math.ceil(wordCount / 238). Count words by splitting on whitespace: text.trim().split(/\\s+/).length. In Ghost CMS, reading time is displayed automatically. In WordPress, use the Yoast SEO plugin or a dedicated reading time plugin.' },
            { q: 'What reading speed should I use for technical documentation?', a: 'Technical documentation is typically read at 150–200 WPM rather than the prose baseline of 238 WPM, because readers must process code syntax, unfamiliar terms, and conceptual diagrams that have no analogue in narrative text. For API reference documentation, users are usually scanning for a specific function or parameter — they are not reading end-to-end at all, making reading time less meaningful. For tutorial-style documentation where users follow steps sequentially, a 200 WPM baseline produces a more realistic estimate.' },
            { q: 'Is reading time the same as speaking time for podcasts or narration?', a: 'No — speaking time is significantly lower than reading time. The typical narration speed for audiobooks and podcasts is 130–160 WPM, compared to 238 WPM for silent reading. This means a 1,000-word article that takes 4 minutes to read silently takes approximately 6–8 minutes to narrate comfortably. For conference talks and presentations, 120–140 WPM is the norm for clearly paced delivery. When writing a speech or script, calculate your speaking time using 140 WPM as the default to avoid running over your allotted slot.' },
            { q: 'How do images and media affect reading time?', a: 'Pure word-count calculators do not account for images, videos, charts, or interactive elements. Medium historically adds time for images: 12 seconds for the first image, decreasing to 3 seconds per additional image. In practice, complex infographics and data visualisations can add 30–60 seconds each for readers who study them carefully. Interactive demos and embedded videos should be counted by their estimated viewing/interaction time. For content-heavy pages with many visuals, the actual time-on-page can be 30–50% longer than the word-count-only reading time estimate suggests.' },
          ].map(({ q, a }, i, arr) => (
